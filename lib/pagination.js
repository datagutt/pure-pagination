(function(module) {
	...
	SearchPaginator.prototype.render = function() {
		...
		if(result.previous) {
			html += '<a href="' + prelink + result.previous + '" class="pure-button prev">' + this.options.translator('PREVIOUS') + '</a>';
		}
		...
		if(result.next) {
			html += '<a href="' + prelink + result.next + '" class="pure-button next>' + this.options.translator('NEXT') + '</a>';
		}
		html += '</ul>';
		return html;
	};
